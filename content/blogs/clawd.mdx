...
- **other tips.**
  ...
  - **git worktrees**. [claude guide](https://code.claude.com/docs/en/common-workflows). create a new worktree for each feature and then merge it back to the main/create a PR. helps to have multiple agents running at the same time  
  - **custom subagents**. you can define a custom subagent using markdown that does a specific thing (eg code simplifier or reviewer) [opencode's guide](https://opencode.ai/docs/agents/) is great.
...
**building on top of an coding agent**. in a sense, if you wanted to build your own workflows, an often major bottleneck is figuring out the orchestration as well as tool, mcp and search capabilities of your agent. most coding agents now have their own SDK you can build on top off, so you have these ready made pasta sauce for your agentic applications. i haven't properly looked at the economics of doing so and realize doing so probably means slower results.[github's guide](https://github.blog/news-insights/company-news/build-an-agent-into-any-app-with-the-github-copilot-sdk/https://github.blog/news-insights/company-news/build-an-agent-into-any-app-with-the-github-copilot-sdk/), [opencode SDK projects](https://opencode.ai/docs/sdk/).
...
to add skills, vercel has made it super easy with their 'npx skills add' [repo](https://github.com/vercel-labs/skills?tab=readme-ov-file). it works with 36 coding agents and you can target it for the user vs project or for only a specific agent.
...
- [superpowers](https://github.com/obra/superpowers). one stop shop for a agentic swe
