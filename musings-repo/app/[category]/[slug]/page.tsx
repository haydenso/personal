...
export default async function MusingFullPage({ params }: Props) {
  ...
  const musing = musings.find((m) => m.slug === slug && (m.category || config.defaultCategory) === category)
  
  if (!musing) {
    return (
      <main className="p-12">
        <h1 className="text-2xl font-bold mb-4">Musing not found</h1>
        <Link href="/" className="text-blue-600 hover:underline">← Back to all musings</Link>
      </main>
    )
  }
        ...
        <div className="flex flex-col h-[calc(100vh-6rem)] md:min-h-screen bg-white rounded-xl md:rounded-none shadow-lg md:shadow-none overflow-hidden">
          ...
          <div className="bg-[#f9f9f9] h-11 flex items-center justify-between px-4 border-b border-[#e5e5e5] text-sm text-[#666]">
            <div className="flex items-center gap-3">
              <Link href="/" className="flex items-center gap-1 p-1 text-[#e0b326] hover:text-[#c4a825] text-lg">
...
// Generate static params for all musings
export async function generateStaticParams() {
  return musings.map((musing) => ({
    category: musing.category || config.defaultCategory,
    slug: musing.slug,
  }))
}
